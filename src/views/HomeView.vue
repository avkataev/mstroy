...
import { ref } from 'vue'
import { AllCommunityModule, ModuleRegistry, type ValueFormatterParams } from 'ag-grid-community'
import { TreeDataModule, LicenseManager, RowGroupingPanelModule } from 'ag-grid-enterprise'
import { type TreeItem, type TreeItemForAgGrid, TreeStore } from '@/classes/TreeStore.ts'
...
const colDefs = ref([
  ...
  },
  {
    ...
    valueFormatter: (item: ValueFormatterParams<TreeItemForAgGrid>) => {
      return itemsInit?.getChildren(item.data?.id)?.length ? 'Категория' : 'Элемент'
    },
  },
  {
    field: 'label',
